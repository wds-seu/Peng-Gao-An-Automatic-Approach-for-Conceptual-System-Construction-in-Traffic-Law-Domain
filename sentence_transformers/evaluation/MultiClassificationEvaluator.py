...
class MultiClassificationEvaluator(SentenceEvaluator):
    """
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        seq_len = len(embeddings1[0])
        # print('MultiClassificationEvaluator seq_len size:', seq_len)
        embeddings1 = np.reshape(embeddings1, (-1, self.sentence_embedding_dimension))
        lin = np.matmul(embeddings1,
                        np.reshape(bilinear_W,
                                   (self.sentence_embedding_dimension, -1)))
        bilin = np.matmul(np.reshape(lin, (new_batch_size, seq_len*self.num_labels, -1)),
                          np.transpose(embeddings2, (0, 2, 1)))
        bilin = np.reshape(bilin, (-1, self.num_labels, seq_len))
        bilin = np.reshape(bilin, (new_batch_size, seq_len, self.num_labels, seq_len))
        result = self.softmax(bilin, axis=2)
        result = np.transpose(result, (0, 1, 3, 2))
        predictions_list = self.logsumexp(result, 1)
        predictions_list = self.logsumexp(predictions_list, 1)
